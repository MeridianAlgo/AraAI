...
def store_data(data_dir, db_file, mode='full'):
    ...
    for csv_file in csv_files:
        ...
        try:
            ...
            insert_df = df[[
                ...
                'Open', 'High', 'Low', 'Close', 'Volume', 'FetchDate'
            ]].copy()
            
            insert_df.columns = [
                'symbol', 'asset_type', 'date',
                'open', 'high', 'low', 'close', 'volume', 'fetch_date'
            ]
            
            # Insert or replace data
            if mode == 'full':
                # Full mode: replace existing data
                insert_df.to_sql('market_data', conn, if_exists='append', index=False)
            else:
                # Refresh mode: only insert new data
                for _, row in insert_df.iterrows():
                    try:
                        conn.execute('''
                            INSERT OR IGNORE INTO market_data 
                            (symbol, asset_type, date, open, high, low, close, volume, fetch_date)
                            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
                        ''', tuple(row))
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO training_runs (run_date, mode, symbols_count, rows_processed, status)
        VALUES (?, ?, ?, ?, ?)
    ''', (datetime.now(), mode, len(symbols_processed), total_rows, 'completed'))
...
def main():
    ...
    # Store data
    store_data(args.data_dir, args.db_file, args.mode)
