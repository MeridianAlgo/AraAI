...
class PatternRecognition:
    ...
    def three_white_soldiers(data: pd.DataFrame) -> pd.DataFrame:
        ...
        is_pattern = (
            (body_0 > 0) & (body_1 > 0) & (body_2 > 0) &  # All bullish
    ...
    def three_black_crows(data: pd.DataFrame) -> pd.DataFrame:
        ...
        is_pattern = (
            (body_0 < 0) & (body_1 < 0) & (body_2 < 0) &  # All bearish
    ...
    @staticmethod
    def double_bottom(data: pd.DataFrame, window: int = 20, tolerance: float = 0.02) -> pd.DataFrame:
        """
    ...
    def triangle_pattern(data: pd.DataFrame, window: int = 20) -> pd.DataFrame:
        ...
        high_slope = result['high'].rolling(window=window).apply(
            lambda x: np.polyfit(np.arange(len(x)), x, 1)[0] if len(x) == window else 0,
            raw=True
        )
        
        low_slope = result['low'].rolling(window=window).apply(
            lambda x: np.polyfit(np.arange(len(x)), x, 1)[0] if len(x) == window else 0,
            raw=True
        )
        
        # Ascending triangle: flat highs, rising lows
        result['pattern_ascending_triangle'] = ((np.abs(high_slope) < 0.01) & (low_slope > 0.01)).astype(int)
        
        # Descending triangle: falling highs, flat lows
        result['pattern_descending_triangle'] = ((high_slope < -0.01) & (np.abs(low_slope) < 0.01)).astype(int)
        
        # Symmetrical triangle: converging highs and lows
        result['pattern_symmetrical_triangle'] = ((high_slope < -0.01) & (low_slope > 0.01)).astype(int)
    ...
    def wedge_pattern(data: pd.DataFrame, window: int = 20) -> pd.DataFrame:
        ...
        result['pattern_rising_wedge'] = ((high_slope > 0) & (low_slope > 0) & (high_slope < low_slope * 2)).astype(int)
        
        # Falling wedge: both falling, converging
        result['pattern_falling_wedge'] = ((high_slope < 0) & (low_slope < 0) & (high_slope > low_slope * 2)).astype(int)
        
        return result
    
    @staticmethod
    def channel_pattern(data: pd.DataFrame, window: int = 20, tolerance: float = 0.02) -> pd.DataFrame:
        """
...
def register_pattern_indicators():
    ...
    registry.register(
        ...
        output_columns=[
            ...
            'pattern_head_shoulders', 'pattern_double_top', 'pattern_double_bottom',
            'pattern_ascending_triangle', 'pattern_descending_triangle', 'pattern_symmetrical_triangle',
            'pattern_rising_wedge', 'pattern_falling_wedge',
            'pattern_channel', 'pattern_cup_handle'
        ]
    )
