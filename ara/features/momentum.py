...
class MomentumIndicators:
    ...
    def roc(data: pd.DataFrame, period: int = 12, column: str = 'close') -> pd.DataFrame:
        ...
        result[f'roc_{period}'] = ((result[column] - result[column].shift(period)) / 
                                    result[column].shift(period)) * 100
    ...
    def ultimate_oscillator(
        ...
        bp = result['close'] - pd.concat([result['low'], result['close'].shift()], axis=1).min(axis=1)
        ...
        result['ultimate_osc'] = 100 * ((weight1 * avg1) + (weight2 * avg2) + (weight3 * avg3)) / (weight1 + weight2 + weight3)
    ...
    def awesome_oscillator(data: pd.DataFrame, fast: int = 5, slow: int = 34) -> pd.DataFrame:
        ...
        # Calculate AO
        result['awesome_osc'] = median_price.rolling(window=fast).mean() - median_price.rolling(window=slow).mean()
    ...
    def kama(
        ...
        volatility = pd.Series(np.abs(np.diff(prices, prepend=prices[0]))).rolling(window=period).sum().values
    ...
    @staticmethod
    def inertia(data: pd.DataFrame, period: int = 20, rvi_period: int = 14, column: str = 'close') -> pd.DataFrame:
        """
        Inertia Indicator.
        ...
        result['inertia'] = result['rvi'].rolling(window=rvi_period).apply(
            lambda x: np.polyfit(np.arange(len(x)), x, 1)[0] if len(x) == rvi_period else np.nan,
            raw=True
        )
...
def register_momentum_indicators():
    ...
    registry.register(
        ...
        parameters={'period1': 7, 'period2': 14, 'period3': 28, 'weight1': 4.0, 'weight2': 2.0, 'weight3': 1.0},
        required_columns=['high', 'low', 'close'],
    ...
    registry.register(
        ...
        parameters={'long_period': 25, 'short_period': 13, 'signal_period': 13, 'column': 'close'},
        required_columns=['close'],
