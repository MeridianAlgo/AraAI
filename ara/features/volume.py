...
class VolumeIndicators:
    ...
    def obv(data: pd.DataFrame) -> pd.DataFrame:
        ...
        # Calculate OBV
        obv = np.where(price_change > 0, result['volume'],
    ...
    def vwap(data: pd.DataFrame) -> pd.DataFrame:
        ...
        if len(result) > 100:  # Assume daily data, use rolling
            period = 20
            result['vwap'] = (typical_price * result['volume']).rolling(window=period).sum() / result['volume'].rolling(window=period).sum()
    ...
    def volume_roc(data: pd.DataFrame, period: int = 12) -> pd.DataFrame:
        ...
        result['volume_roc'] = ((result['volume'] - result['volume'].shift(period)) / 
                                result['volume'].shift(period)) * 100
    ...
    def ease_of_movement(data: pd.DataFrame, period: int = 14) -> pd.DataFrame:
        ...
        distance = ((result['high'] + result['low']) / 2) - ((result['high'].shift() + result['low'].shift()) / 2)
    ...
    def negative_volume_index(data: pd.DataFrame) -> pd.DataFrame:
        ...
        for i in range(1, len(result)):
            if result['volume'].iloc[i] < result['volume'].iloc[i-1]:
                ...
                price_change = (result['close'].iloc[i] - result['close'].iloc[i-1]) / result['close'].iloc[i-1]
    ...
    def volume_weighted_macd(
        ...
        # Calculate EMAs
        fast_ema = vw_price.ewm(span=fast, adjust=False).mean() / result['volume'].ewm(span=fast, adjust=False).mean()
        slow_ema = vw_price.ewm(span=slow, adjust=False).mean() / result['volume'].ewm(span=slow, adjust=False).mean()
