...
class VolatilityIndicators:
    ...
    @staticmethod
    def chaikin_volatility(data: pd.DataFrame, period: int = 10, roc_period: int = 10) -> pd.DataFrame:
        """
        Chaikin Volatility.
        ...
        result['chaikin_volatility'] = ((ema_range - ema_range.shift(roc_period)) / 
                                        ema_range.shift(roc_period)) * 100
    ...
    def price_channels(data: pd.DataFrame, period: int = 20) -> pd.DataFrame:
        ...
        result['price_channel_middle'] = (result['price_channel_upper'] + result['price_channel_lower']) / 2
    ...
    @staticmethod
    def mass_index_volatility(data: pd.DataFrame, period: int = 9, sum_period: int = 25) -> pd.DataFrame:
        """
...
def register_volatility_indicators():
    ...
    registry.register(
        ...
        parameters={'period': 20},
        required_columns=['high', 'low'],
        output_columns=['price_channel_upper', 'price_channel_lower', 'price_channel_middle']
    )
