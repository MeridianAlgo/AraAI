...
class TrendIndicators:
    ...
    def wma(data: pd.DataFrame, period: int = 20, column: str = 'close') -> pd.DataFrame:
        ...
        result[f'wma_{period}'] = result[column].rolling(window=period).apply(weighted_mean, raw=True)
        return result
    ...
    def supertrend(
        ...
        for i in range(period, len(result)):
            if upper_band.iloc[i] < final_upper.iloc[i-1] or result['close'].iloc[i-1] > final_upper.iloc[i-1]:
                final_upper.iloc[i] = upper_band.iloc[i]
            else:
                final_upper.iloc[i] = final_upper.iloc[i-1]
            
            if lower_band.iloc[i] > final_lower.iloc[i-1] or result['close'].iloc[i-1] < final_lower.iloc[i-1]:
        ...
        for i in range(period, len(result)):
            if i == period:
                ...
                if supertrend.iloc[i-1] == final_upper.iloc[i-1] and result['close'].iloc[i] <= final_upper.iloc[i]:
                    supertrend.iloc[i] = final_upper.iloc[i]
                    trend.iloc[i] = -1
                elif supertrend.iloc[i-1] == final_upper.iloc[i-1] and result['close'].iloc[i] > final_upper.iloc[i]:
                    supertrend.iloc[i] = final_lower.iloc[i]
                    trend.iloc[i] = 1
                elif supertrend.iloc[i-1] == final_lower.iloc[i-1] and result['close'].iloc[i] >= final_lower.iloc[i]:
                    supertrend.iloc[i] = final_lower.iloc[i]
                    trend.iloc[i] = 1
                else:
                    supertrend.iloc[i] = final_upper.iloc[i]
    ...
    def aroon(data: pd.DataFrame, period: int = 25) -> pd.DataFrame:
        ...
        aroon_up = result['high'].rolling(window=period + 1).apply(
            lambda x: (period - (len(x) - 1 - np.argmax(x))) / period * 100,
            raw=True
        )
        
        # Aroon Down: ((period - periods since lowest low) / period) * 100
        aroon_down = result['low'].rolling(window=period + 1).apply(
    ...
    @staticmethod
    def trix(data: pd.DataFrame, period: int = 15, signal: int = 9, column: str = 'close') -> pd.DataFrame:
        """
...
def register_trend_indicators():
    ...
    registry.register(
        ...
        required_columns=['high', 'low', 'close'],
        output_columns=['ichimoku_tenkan', 'ichimoku_kijun', 'ichimoku_senkou_a', 'ichimoku_senkou_b', 'ichimoku_chikou']
    )
    ...
    registry.register(
        ...
        parameters={'roc1': 10, 'roc2': 15, 'roc3': 20, 'roc4': 30, 
                   'sma1': 10, 'sma2': 10, 'sma3': 10, 'sma4': 15, 'signal': 9, 'column': 'close'},
        required_columns=['close'],
