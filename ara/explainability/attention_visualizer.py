...
class AttentionVisualizer:
            ...
            def hook(module, input, output):
                ...
                if isinstance(output, tuple) and len(output) > 1:
                    ...
                    if hasattr(module, 'attention_weights'):
                        self.attention_weights[name] = module.attention_weights.detach().cpu().numpy()
            return hook
    ...
    def get_attention_summary(
        ...
        if attention_weights.ndim != 3:
            raise ValidationError(
                "attention_weights must be 3D or 4D",
        ...
        attention_entropy = -np.sum(
            avg_attention * np.log(avg_attention + epsilon),
            axis=1
        )
    ...
    def create_attention_heatmap_data(
        ...
        return {
            ...
            'title': f"Attention Heatmap (Head {head_idx})" if head_idx is not None else "Average Attention Heatmap",
            'xlabel': 'Key Position',
