...
class BacktestEngine:
    ...
    def run_backtest(
        ...
        if len(holdout_data) > 0:
            holdout_metrics = self._test_on_holdout(
                holdout_data, model_predict_fn, feature_columns, target_column, price_column
            )
    ...
    def _walk_forward_validation(
        ...
        while start_idx + test_window <= len(data):
            ...
            dates = test_subset.index.tolist() if isinstance(test_subset.index, pd.DatetimeIndex) else list(range(len(test_subset)))
    ...
    def _cross_validation(
        ...
        for i in range(n_folds):
            ...
            dates = test_data.index.tolist() if isinstance(test_data.index, pd.DatetimeIndex) else None
    ...
    def save_results(
        ...
        report = self.reporter.generate_full_report(
            ...
            regime_performance={k: v.to_dict() for k, v in result.regime_performance.items()} if result.regime_performance else None
        )
