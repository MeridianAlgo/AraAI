...
class DeFiDataProvider:
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    logger.info(
                        f"Fetched {len(protocols)} DeFi protocols",
                        count=len(protocols)
                    )
    ...
    async def fetch_lending_rates(
        ...
        try:
            ...
            raise DataProviderError(
                f"Failed to fetch lending rates",
    ...
    async def fetch_liquidation_data(
        ...
        try:
            ...
            raise DataProviderError(
                f"Failed to fetch liquidation data",
                {"error": str(e)}
            )
        ...
        try:
            ...
            async with self.session.get(url) as response:
                if response.status == 200:
                    ...
                    if stablecoin:
                        stablecoin_data = [
                            s for s in data.get('peggedAssets', [])
                        ...
                        if stablecoin_data:
                            ...
                            async with self.session.get(hist_url, params={'stablecoin': coin_id}) as hist_response:
    ...
    def _generate_sample_tvl(self, name: str, days: int = 365) -> pd.DataFrame:
        ...
        logger.warning(
            f"Using sample TVL data for {name}",
            name=name
        )
    ...
    def calculate_defi_metrics(
        ...
        if 'tvl' in tvl_df.columns:
            ...
            metrics['tvl_momentum'] = (
                ...
                tvl_df['tvl'].rolling(window=30).mean() - 1
            )
        ...
        if lending_rates:
            ...
            metrics['rate_spread'] = (
                lending_rates.get('borrow_apy', 0) -
                ...
                if response.status == 200:
                    ...
                    if tvl_data:
                        ...
                        overview = {
                            ...
                            'tvl_change_30d': (latest_tvl - prev_tvl) / prev_tvl if prev_tvl > 0 else 0,
                            'timestamp': datetime.now(),
    ...
    def calculate_defi_risk_score(
        ...
        if 'liquidation_volume_usd' in liquidation_df.columns:
            liq_norm = (
                (liquidation_df['liquidation_volume_usd'] - liquidation_df['liquidation_volume_usd'].min()) /
                (liquidation_df['liquidation_volume_usd'].max() - liquidation_df['liquidation_volume_usd'].min())
            )
        ...
        if 'at_risk_positions' in liquidation_df.columns:
            risk_norm = (
                ...
                (liquidation_df['at_risk_positions'].max() - liquidation_df['at_risk_positions'].min())
            )
