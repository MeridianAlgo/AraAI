...
class OptimizationRequest(BaseModel):
    ...
    optimization_method: str = Field("mpt", description="Optimization method: mpt, black_litterman, risk_parity, kelly")
    constraints: Optional[Dict] = Field(None, description="Portfolio constraints")
    expected_returns: Optional[Dict[str, float]] = Field(None, description="Expected returns for each asset")
...
async def rebalance_portfolio(
    ...
    try:
        ...
        for symbol in set(list(request.current_weights.keys()) + list(request.target_weights.keys())):
