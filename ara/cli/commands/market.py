...
@click.option('--sources', '-s', multiple=True, 
              ...
              default=['all'], help='Sentiment sources')
@click.option('--timeframe', '-t', type=str, default='24h', 
              help='Timeframe (1h, 24h, 7d)')
...
def correlations(symbols, window, heatmap):
    ...
    try:
        ...
        if pairs:
            ...
            for pair in pairs:
                console.print(f"  {pair['asset1']} - {pair['asset2']}: "
...
@click.argument('symbol')
@click.option('--category', '-c', type=click.Choice(['trend', 'momentum', 'volatility', 'volume', 'all']),
              default='all', help='Indicator category')
...
def _display_indicators(result, category):
    ...
    for indicator, data in result.items():
        signal_color = "green" if data['signal'] == 'BUY' else "red" if data['signal'] == 'SELL' else "yellow"
        table.add_row(
            indicator,
            f"{data['value']:.4f}",
