...
class TwitterSentimentAnalyzer(SentimentAnalyzer):
    ...
    def _validate_config(self) -> None:
        ...
        if not self.bearer_token:
            logger.warning("Twitter bearer token not provided. Twitter sentiment analysis will be disabled.")
    ...
    async def analyze(self, symbol: str, lookback_hours: int = 24) -> SentimentScore:
        ...
        try:
            ...
            return SentimentScore(
                ...
                source=SentimentType.TWITTER,
                metadata={
                    'influencer_tweets': sum(1 for t in tweets if t.get('followers', 0) >= self.min_followers),
