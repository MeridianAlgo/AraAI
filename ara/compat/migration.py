    ...
    def _migrate_pickle_model(
        ...
        try:
            ...
            metadata = {
                ...
                "model_type": type(old_model).__name__ if hasattr(old_model, '__name__') else "unknown"
            }
...
class DataMigrator:
    ...
    def migrate_cache_directory(
        ...
        if not old_cache_dir.exists():
            ...
            return {
                "success": False,
                "error": "Old cache directory not found"
            }
        ...
        # Create backup if requested
        if backup:
            backup_dir = old_cache_dir.parent / f"{old_cache_dir.name}_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
            if self.verbose:
                print(f"Creating backup: {backup_dir}")
    ...
    def validate_migration(self, new_dir: Path) -> Dict[str, Any]:
        ...
        if not new_dir.exists():
            return {
                "valid": False,
                "error": "New directory does not exist"
            }
