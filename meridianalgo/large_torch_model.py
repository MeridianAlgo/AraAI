...
class LargeEnsembleModel(nn.Module):
    """
    Large ensemble model with 1M+ parameters
    Deep architecture for maximum accuracy
    """
    def __init__(self, input_size=44, hidden_sizes=[512, 256, 128, 64], dropout=0.3):
        ...
        # Primary heads (tree-based model simulation)
        ...
        # Secondary heads (linear model simulation)
        self.ridge_head = nn.Linear(final_hidden, 1)
        self.elastic_head = nn.Linear(final_hidden, 1)
        ...
        if self.model_path.exists():
            try:
                ...
                self.model = LargeEnsembleModel(
                    ...
                    hidden_sizes=checkpoint.get('hidden_sizes', [512, 256, 128, 64]),
