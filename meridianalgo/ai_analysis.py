...
class LightweightAIAnalyzer:
    ...
    def analyze_company_with_ai(self, symbol, basic_analysis=None):
        ...
        try:
            ...
            analysis['overall_score'], analysis['recommendation'] = self._calculate_ai_score(analysis)
    ...
    def _prepare_company_data_for_ai(self, info, hist_data):
        ...
        try:
            ...
            price_change_1d = (current_price - hist_data['Close'].iloc[-2]) / hist_data['Close'].iloc[-2] * 100
            price_change_30d = (current_price - hist_data['Close'].iloc[-31]) / hist_data['Close'].iloc[-31] * 100
            price_change_1y = (current_price - hist_data['Close'].iloc[-253]) / hist_data['Close'].iloc[-253] * 100
            ...
            company_data = {
                'basic_info': {
                    ...
                    'business_summary': info.get('longBusinessSummary', '')[:500]  # Truncate for AI
                },
                'financial_metrics': {
                    'market_cap': info.get('marketCap', 0),
...
Analysis:"""
    ...
    def _parse_ai_response(self, ai_text, company_data):
        ...
        try:
            ...
            for line in lines:
                ...
                elif any(word in line.lower() for word in ['recommendation', 'rating', 'buy', 'sell', 'hold']):
    ...
    def _generate_fallback_analysis(self, company_data):
        ...
        try:
            ...
            if profit_margin > 0.1 and revenue_growth > 0.05 and pe_ratio < 20:
                recommendation = {"action": "BUY", "confidence": 0.7, "reasoning": "Good fundamentals and reasonable valuation"}
                price_targets = {
                ...
                recommendation = {"action": "SELL", "confidence": 0.6, "reasoning": "Weak fundamentals"}
                price_targets = {
                ...
                recommendation = {"action": "HOLD", "confidence": 0.6, "reasoning": "Mixed signals"}
                price_targets = {
                    'bull_case': current_price * 1.10,
            ...
            return {
                ...
                'recommendation': {"action": "HOLD", "confidence": 0.5, "reasoning": "Insufficient data"},
                'price_targets': {},
    ...
    def _prepare_enhanced_company_context(self, info, hist_data):
        ...
        try:
            ...
            current_price = hist_data['Close'].iloc[-1]
            price_change_1d = (current_price - hist_data['Close'].iloc[-2]) / hist_data['Close'].iloc[-2] * 100
            price_change_7d = (current_price - hist_data['Close'].iloc[-8]) / hist_data['Close'].iloc[-8] * 100
            price_change_30d = (current_price - hist_data['Close'].iloc[-31]) / hist_data['Close'].iloc[-31] * 100
            price_change_90d = (current_price - hist_data['Close'].iloc[-91]) / hist_data['Close'].iloc[-91] * 100
            price_change_1y = (current_price - hist_data['Close'].iloc[-253]) / hist_data['Close'].iloc[-253] * 100
    ...
    def _analyze_sentiment(self, context):
        ...
        try:
            if not self.sentiment_analyzer:
                return {"sentiment": "neutral", "confidence": 0.5, "reasoning": "AI model not available"}
    ...
    def _generate_insights(self, context):
        ...
        try:
            ...
            prompts = [
                f"Based on the financial data, this company shows",
                f"The investment potential of this stock is",
                f"Key risks to consider include"
            ]
    ...
    def _analyze_growth_with_ai(self, context):
        ...
        try:
            ...
            return {
                ...
                "growth_reasoning": self._generate_growth_reasoning(result['labels'][0], result['scores'][0])
            }
    ...
    def _generate_recommendation(self, context):
        ...
        try:
            if not self.financial_classifier:
                return {"recommendation": "HOLD", "confidence": 0.5, "reasoning": "AI model not available"}
            ...
            return {"recommendation": "HOLD", "confidence": 0.5, "reasoning": f"Error: {str(e)}"}
    ...
    def _calculate_basic_metrics(self, info, hist_data):
        ...
        try:
            ...
            # Price performance
            price_1m = (current_price - hist_data['Close'].iloc[-21]) / hist_data['Close'].iloc[-21] * 100
            price_3m = (current_price - hist_data['Close'].iloc[-63]) / hist_data['Close'].iloc[-63] * 100
            price_1y = (current_price - hist_data['Close'].iloc[0]) / hist_data['Close'].iloc[0] * 100
            ...
            return {
                ...
                "recent_volume_trend": "increasing" if recent_volume > avg_volume else "decreasing",
                "pe_ratio": info.get('trailingPE', 0),
                "market_cap": info.get('marketCap', 0),
                "profit_margin": info.get('profitMargins', 0),
                "debt_to_equity": info.get('debtToEquity', 0)
    ...
    def _fallback_analysis(self, symbol, basic_analysis):
        ...
        return {
            "symbol": symbol,
            "ai_sentiment": {"sentiment": "neutral", "confidence": 0.5, "reasoning": "AI models not available"},
            "ai_insights": ["Basic analysis available", "AI features require transformers library"],
            "ai_recommendation": {"recommendation": "HOLD", "confidence": 0.5, "reasoning": "Limited analysis available"},
            "overall_score": 50.0,
            "recommendation": "HOLD",
...
class ModelManager:
    ...
    def _generate_comprehensive_insights(self, context, info):
        ...
        try:
            ...
            if profit_margin > 0.2:
                insights.append(f"Exceptional profitability with {profit_margin*100:.1f}% profit margin, indicating strong pricing power and operational efficiency.")
            elif profit_margin > 0.1:
                insights.append(f"Solid profitability with {profit_margin*100:.1f}% profit margin, showing good business fundamentals.")
            elif profit_margin > 0:
                insights.append(f"Moderate profitability at {profit_margin*100:.1f}% profit margin, room for improvement in operational efficiency.")
            else:
                insights.append("Company is currently unprofitable, focus on path to profitability and cash burn rate.")
            
            # Growth insights
            if revenue_growth > 0.2:
                insights.append(f"Strong revenue growth of {revenue_growth*100:.1f}% indicates expanding market share and business momentum.")
            elif revenue_growth > 0.1:
                insights.append(f"Healthy revenue growth of {revenue_growth*100:.1f}% shows steady business expansion.")
            elif revenue_growth > 0:
                insights.append(f"Modest revenue growth of {revenue_growth*100:.1f}% suggests stable but slow-growing business.")
            else:
                insights.append("Declining revenues require attention to competitive position and market dynamics.")
            
            # Financial health insights
            if debt_to_equity and debt_to_equity < 0.3:
                insights.append("Conservative debt levels provide financial flexibility and lower risk profile.")
            elif debt_to_equity and debt_to_equity > 1.0:
                insights.append("High debt levels require monitoring of interest coverage and refinancing risks.")
            
            # ROE insights
            if roe > 0.15:
                insights.append(f"Excellent return on equity of {roe*100:.1f}% demonstrates efficient use of shareholder capital.")
            elif roe > 0.1:
                insights.append(f"Good return on equity of {roe*100:.1f}% shows solid management performance.")
            
            # Valuation insights
            pe_ratio = info.get('trailingPE', 0)
            if pe_ratio and pe_ratio < 15:
                insights.append("Attractive valuation with low P/E ratio may indicate undervaluation or value opportunity.")
            elif pe_ratio and pe_ratio > 30:
                insights.append("High P/E ratio suggests growth expectations or potential overvaluation - verify growth prospects.")
            
            # Market position insights
            market_cap = info.get('marketCap', 0)
            if market_cap > 100_000_000_000:
                insights.append("Large-cap stability with established market position and likely dividend potential.")
            elif market_cap < 2_000_000_000:
                insights.append("Small-cap growth potential with higher volatility and risk-reward profile.")
            
            # Sector-specific insights
            sector = info.get('sector', '')
            if sector == 'Technology':
                insights.append("Technology sector exposure provides growth potential but requires monitoring of innovation cycles and competition.")
            elif sector == 'Healthcare':
                insights.append("Healthcare sector offers defensive characteristics with demographic tailwinds and regulatory considerations.")
            elif sector == 'Financial Services':
                insights.append("Financial sector performance tied to interest rates, economic cycles, and regulatory environment.")
    ...
    def _assess_comprehensive_risks(self, context, info):
        ...
        try:
            ...
            if debt_to_equity and debt_to_equity > 1.5:
                ...
                risk_factors.append({"type": "Financial Leverage", "level": "Moderate", "impact": 0.5})
            ...
            if beta > 1.5:
                ...
                risk_factors.append({"type": "Market Volatility", "level": "Moderate", "impact": 0.4})
            
            # Sector-specific risks
            sector = info.get('sector', '')
            sector_risks = {
                'Technology': {"type": "Technology Disruption", "level": "Moderate", "impact": 0.5},
                'Energy': {"type": "Commodity Price", "level": "High", "impact": 0.7},
                'Financial Services': {"type": "Regulatory", "level": "Moderate", "impact": 0.5},
                'Healthcare': {"type": "Regulatory", "level": "Moderate", "impact": 0.4},
                'Consumer Discretionary': {"type": "Economic Sensitivity", "level": "Moderate", "impact": 0.6}
            }
    ...
    def _analyze_competitive_position_ai(self, info):
        ...
        try:
            ...
            if profit_margin > 0.2:
                competitive_analysis["competitive_advantages"].append("Superior profitability indicates pricing power")
            if gross_margin > 0.6:
                competitive_analysis["competitive_advantages"].append("High gross margins suggest strong value proposition")
            if roe > 0.2:
                competitive_analysis["competitive_advantages"].append("Excellent capital efficiency")
            ...
            if sector == 'Technology':
                ...
                competitive_analysis["competitive_advantages"].append("Brand loyalty and distribution")
                competitive_analysis["moat_strength"] = "Strong"
            elif sector == 'Utilities':
                competitive_analysis["competitive_advantages"].append("Regulatory barriers and infrastructure")
    ...
    def _analyze_esg_with_ai(self, info):
        ...
        try:
            ...
            if 0.05 < insider_ownership < 0.3:
                ...
                governance_score -= 10
                esg_analysis["esg_risks"].append("High insider control may limit shareholder rights")
            ...
            sector = info.get('sector', '')
            if sector == 'Energy':
                esg_analysis["environmental_factors"].append("Carbon footprint and transition risks")
                esg_analysis["esg_risks"].append("Climate change regulatory risks")
            elif sector == 'Technology':
                esg_analysis["social_factors"].append("Data privacy and digital divide considerations")
    ...
    def _analyze_market_outlook(self, context, info):
        ...
        try:
            ...
            sector_outlooks = {
                'Technology': {
                    "outlook": "Positive",
                    "drivers": ["Digital transformation", "AI adoption", "Cloud migration"],
            ...
            pe_ratio = info.get('trailingPE', 0)
            if pe_ratio and pe_ratio < 15:
                outlook["potential_catalysts"].append("Attractive valuation may attract value investors")
            
            # Financial health catalysts
            free_cash_flow = info.get('freeCashflow', 0)
            if free_cash_flow > 0:
                outlook["potential_catalysts"].append("Positive free cash flow supports dividend/buyback potential")
    ...
    def _get_financial_sentiment(self, company_data):
        ...
        try:
            if not self.sentiment_analyzer:
                return {"sentiment": "neutral", "confidence": 0.5, "reasoning": "Sentiment analyzer not available"}
            ...
            if isinstance(result, list) and len(result) > 0:
                ...
                return {
                    ...
                    "reasoning": f"Financial sentiment analysis based on key metrics",
                    "all_scores": result
                }
            ...
            return {"sentiment": "neutral", "confidence": 0.5, "reasoning": f"Sentiment analysis failed: {str(e)}"}
    ...
    def _classify_investment_risk(self, company_data):
        ...
        try:
            ...
            return {"risk_level": "medium", "confidence": 0.5, "factors": [], "error": str(e)}
    ...
    def _assess_market_position_with_ai(self, info, ai_analysis):
        ...
        try:
            ...
            for insight in ai_insights:
                if any(word in insight.lower() for word in ['advantage', 'leader', 'dominant', 'strong']):
    ...
    def _get_model_info(self):
        ...
        try:
            ...
            if self.main_ai:
                model_info['main_ai_model'] = self.model_configs['mistral_alternative']['model_name']
            if self.sentiment_analyzer:
                model_info['sentiment_model'] = self.model_configs['financial_specialist']['model_name']
            if self.financial_classifier:
    ...
    def _fallback_analysis(self, symbol, basic_analysis):
        ...
        return {
            ...
            "ai_analysis": {
                ...
                "recommendation": {"action": "HOLD", "confidence": 0.5, "reasoning": "Limited analysis available"},
                "price_targets": {},
                "full_analysis": "AI models not available. Install transformers library for comprehensive analysis.",
                "ai_generated": False
            },
            "ai_sentiment": {"sentiment": "neutral", "confidence": 0.5, "reasoning": "AI models not available"},
            "ai_risk_classification": {"risk_level": "medium", "confidence": 0.5, "factors": []},
            "overall_score": 50.0,
            "recommendation": "HOLD",
