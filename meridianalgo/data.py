...
class MarketDataManager:
    ...
    def get_stock_analysis(self, symbol):
        ...
        try:
            ...
            price_change_1w = (current_price - data['Close'].iloc[-6]) / data['Close'].iloc[-6] * 100 if len(data) >= 6 else 0
            price_change_1m = (current_price - data['Close'].iloc[-21]) / data['Close'].iloc[-21] * 100 if len(data) >= 21 else 0
...
class TechnicalIndicators:
    ...
    def get_signal_summary(self, data):
        ...
        try:
            ...
            bullish_count = sum(1 for signal_group in signals.values() 
                              if isinstance(signal_group, dict) 
                              for signal in signal_group.values() 
                              if 'BULLISH' in str(signal))
            
            bearish_count = sum(1 for signal_group in signals.values() 
                              if isinstance(signal_group, dict) 
                              for signal in signal_group.values() 
...
class MarketDataValidator:
    ...
    def validate_data_quality(data):
        ...
        try:
            ...
            return {
                ...
                'quality_grade': 'A' if score >= 90 else 'B' if score >= 80 else 'C' if score >= 70 else 'D' if score >= 60 else 'F'
            }
    ...
    def clean_and_validate(data):
        ...
        try:
            ...
            validation_result['removal_percentage'] = (removed_rows / original_length * 100) if original_length > 0 else 0
