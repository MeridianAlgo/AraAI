...
class StockDataset(Dataset):
    ...
    def prepare_sequences(self, data: np.ndarray) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        for i in range(len(data) - self.sequence_length - self.prediction_window + 1):
            ...
            y.append(data[i + self.sequence_length:i + self.sequence_length + self.prediction_window, 0])  # Predict only the close price
...
class StockPredictorTrainer:
    ...
    def load_model(self, filepath: str) -> None:
        ...
        input_dim = next(iter(self.train_loader))[0].shape[2] if self.train_loader else 5  # Default to 5 if no data loaded
        self.model = StockPredictor(input_dim=input_dim).to(self.device)
...
def train_and_evaluate(ticker: str = 'AAPL', 
                      ...
                      model_dir: str = 'saved_models') -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--prediction_window', type=int, default=1, 
